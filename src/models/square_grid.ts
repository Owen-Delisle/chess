...
export default class SquareGrid {
	...
	public static point_by_piece(piece: Piece): GridPoint | undefined {
		...
		for (
			let row_index = 0;
			row_index < this.square_grid.length;
			row_index++
		) {
			for (
				let col_index = 0;
				col_index < this.square_grid[0].length;
				col_index++
			) {
				if (
					this.piece_title_at_point(row_index, col_index) == piece_id
				) {
	...
	private static piece_title_at_point(row: number, col: number): string {
		if (
			this.square_grid[row][col].piece_attached_to_square() !== undefined
		) {
	...
	public static piece_by_grid_point(point: GridPoint): Piece | undefined {
		...
		return SquareGrid.square_grid[point.row][
			point.col
