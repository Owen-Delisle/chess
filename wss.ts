    ...
    client_connection.on('message', function incoming(message) {
        ...
        switch(data.message_type) {
            ...
            case MessageType.draw:
                send_draw_message_to_recipient(data.recipient_id, data.message)
            break
...
function send_draw_message_to_recipient(recipient_id: UUID, message: string) {
    const data = { type: MessageType.draw.toString(), message }
    const json_data = JSON.stringify(data)
