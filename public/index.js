...
class Pawn extends Piece {
  ...
  move_to(new_pos) {
    return new Promise(async (resolve) => {
      ...
      this.possible_moves = [];
      const point = SquareGrid.point_at_board_position(new_pos);
...
class MoveController {
  ...
  static async move_piece_to(selected_pos, piece8) {
    ...
    GameController.add_move_to_list({ piece: piece8, from: piece8.pos, to: selected_pos });
    await piece8.move_to(selected_pos);
    this.redraw();
  }
  static remove_piece_conditions(selected_pos) {
    const new_square = SquareGrid.square_by_board_position(selected_pos);
    if (!new_square) {
  ...
  static async move_castle_pieces(new_king_pos, king_piece, new_rook_pos, rook_piece) {
...
class GameController {
  ...
  static online_turn_switch() {
    if (this.count <= 0) {
      const piece8 = PieceList.piece_by_position("E7");
      MoveController.move_piece_to("E5", piece8);
      this.count++;
    }
  }
...
var GameType;
(function(GameType2) {
  GameType2["online"] = "online";
  GameType2["offline"] = "offline";
})(GameType || (GameType = {}));
